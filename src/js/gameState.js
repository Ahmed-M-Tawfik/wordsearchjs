...
function wordSearchFillAlgorithm(rows, columns, wordCount, sourceDictionary) {
    ...
    let pickedWords = Array(wordCount);
    for (let i = 0; i < wordCount; i++) {
        let pickedWord = null;
        do {
            let dictionaryIndex = Math.floor(Math.random() * sourceDictionary.length);
            pickedWord = sourceDictionary[dictionaryIndex];
        } while (stringIsBlank(pickedWord) || (pickedWord.length > rows || pickedWord.length > columns) || pickedWords.includes(pickedWord));
        pickedWords[i] = pickedWord;
    }

    console.log("Picked words: " + pickedWords);

    // add picked words to the grid
    for (let i = 0; i < wordCount; i++) {
        let wordToBeAdded = pickedWords[i];
...
export function gameInit(gameConfig) {
    gameState = {
        ...
        grid: wordSearchFillAlgorithm(gameConfig.rows, gameConfig.columns, gameConfig.wordCount, gameConfig.sourceDictionary),
